...
bool UpdatePackegeBuyerName(const char* buyer_name, int serial_number)
{
...
//if the opening faild return 0, if it exsist return 1
...
//if the opening faild return 0, else return 1 
...
//If the file opens fails return 1, if the serial number does not exist returns 0, if the deletion succeeded returns 1

int deletePackegeFromDb(int serial_number)
{
	int index = searchPackegeBySerialNumber(serial_number);
	if (index == -1)
		return 0;
	...
	/*for (int i = 0, j = 0; i < size && j < size-1 ; i++, j++) // copy all the old DB without the specific packege that you eant to delete
	{
		if (source_arr[i].serial_number != serial_number)
...
bool Payment(int serial_number, const char* user_name)
{
